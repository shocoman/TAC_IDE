...
data_flow_framework(Function &f, Flow flow, Meet meet_operator, std::set<T> init_value, F transfer_function) {

    auto &blocks = f.basic_blocks;
    auto &id_to_block = f.id_to_block;

    auto [visit_order, boundary_node] = flow == Flow::Forwards
                                            ? std::pair(f.get_reverse_post_ordering(), f.get_entry_block())
    ...
    while (changed) {
        ...
        for (auto &[id, _] : sorted_visit_order) {
            ...
            auto b = meet_operator == Meet::Intersection ? intersection_of_sets(preds) : union_of_sets(preds);
            B[id] = b;
