...
#line 10 "parser/grammar/parser.y"

    #include <string>

    class driver;

#line 55 "parser/parser.hpp"
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::TOK_YYEOF || tok == token::TOK_YYerror || tok == token::TOK_YYUNDEF || tok == token::TOK_ASSIGN || tok == token::TOK_PLUS || tok == token::TOK_MINUS || tok == token::TOK_MULT || tok == token::TOK_DIV || tok == token::TOK_LPAREN || tok == token::TOK_RPAREN || tok == token::TOK_SEMI);
      }
#else
